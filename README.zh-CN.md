...
核心思路：解析 OpenAPI 规范 → 生成确定性测试场景 → 构造 HTTP 请求 → 真实执行 → 响应验证 → JSON 报告。
...
| 功能 | 状态 | 说明 |
|------|------|------|
| Natural Language Test Intent | 🔜 计划中 | 用自然语言描述测试意图，自动生成场景 |
| LLM Semantic Planner | 🔜 计划中 | 基于 LLM 的智能场景规划，补充确定性生成的盲区 |
